      ...
      <div className="w-full h-full relative z-10">
        {!selectedRole ? (
          <div className="min-h-screen flex items-center justify-center p-6 animate-in fade-in zoom-in-95 duration-1000">
            <div className="w-full max-w-6xl space-y-12">
              <div className="text-center space-y-4">
                ...
                <p className="text-white/60 font-bold uppercase tracking-[0.4em] text-[10px] md:text-[20px]">Unified Academic Access System</p>
              </div>
              ...
              <div className="flex flex-wrap justify-center gap-4 px-4 w-full max-w-full mx-auto">
                {roles.map(role => (
                  ...
                  >
                    ...
                    <h3 className="text-lg md:text-[24px] font-black text-white mb-2">{role.title}</h3>
                    <p className="text-[11px] md:text-[19px] text-white/60 font-semibold leading-relaxed mb-8">{role.desc}</p>
                    <div className="mt-auto flex items-center gap-2 text-school-gold font-black text-[9px] md:text-[14px] uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          <div className="min-h-screen flex flex-col md:flex-row animate-in slide-in-from-right duration-500 overflow-hidden">
            <div className={`md:w-5/12 relative bg-gradient-to-br ${currentRoleConfig?.color} bg-opacity-90 flex flex-col justify-center p-8 md:p-12 lg:p-24 text-white overflow-hidden shadow-2xl`}>
              <div className="absolute inset-0 opacity-10 pointer-events-none">
                <img src={LOGO_URL} className="absolute -bottom-40 -right-40 rotate-12 w-[600px] grayscale brightness-200" alt="" />
              </div>
              <div className="relative z-10">
                <button onClick={() => setSelectedRole(null)} className="text-white/60 hover:text-white font-bold text-sm md:text-base flex items-center gap-3 mb-10 md:mb-16 transition-colors uppercase tracking-widest group">
                  <ArrowRight className="rotate-180 group-hover:-translate-x-1 transition-transform" size={22} /> Exit Portal
                </button>
                <div className="w-16 h-16 md:w-24 md:h-24 bg-white p-2 md:p-3 rounded-[1.5rem] md:rounded-[2rem] flex items-center justify-center mb-6 md:mb-10 shadow-2xl animate-float overflow-hidden">
                  <img src={LOGO_URL} alt="Sto. Niño Logo" className="w-full h-full object-contain" />
                </div>
                <h2 className="text-3xl md:text-6xl font-black uppercase tracking-tighter leading-none mb-4 md:mb-8">
                  {selectedRole.toLowerCase()} <br /> Authorization
                </h2>
                <p className="text-base md:text-xl text-white/80 font-medium max-w-sm leading-relaxed">
                  Protecting learner records and institutional integrity. Identity verification is required.
                </p>
              </div>
            </div>

            <div className="md:w-7/12 flex items-center justify-center p-6 md:p-8 lg:p-24">
              <div className="w-full max-w-md space-y-8 md:space-y-12">
                <div className="space-y-2 md:space-y-4">
                  <div className="flex items-center gap-3">
                    <ShieldCheck className="text-school-gold" size={28} />
                    <h3 className="text-3xl md:text-4xl font-black text-white tracking-tighter uppercase">
                      {isSignup ? 'Create Profile' : 'Authenticate'}
                    </h3>
                  </div>
                  <p className="text-white/50 font-medium text-sm md:text-lg">
                ...
                {isSuspended ? (
                  ...
                  <form className="space-y-6 md:space-y-8" onSubmit={handleAction}>
                    {error && (
